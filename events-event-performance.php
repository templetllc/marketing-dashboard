...
<body class="has-navbar-vertical-aside navbar-vertical-aside-show-xl">
        ...
        */
                    ...
                    for (var i = 0, len = eventList.length; i < len; i++) {//console.log(collateralList[i]['Campaign']);
                        ...
                        if($.inArray(eventList[i]['Country'], countryList)==-1){
                            ...
                            if($.inArray(eventList[i]['Reporting Business Unit'], buList)==-1){
                                buList.push(eventList[i]['Reporting Business Unit']);
                                $("#buSelect").append('<option>' + eventList[i]['Reporting Business Unit'] + '</option>');
                            }
                            if($.inArray(eventList[i]['Sub-Category'], subcatList)==-1){
                                subcatList.push(eventList[i]['Sub-Category']);
                                $("#subcatSelect").append('<option>' + eventList[i]['Sub-Category'] + '</option>');
                            }
                            if($.inArray(eventList[i]['_campaignMonthStr'], monthList)==-1){
                                monthList.push(eventList[i]['_campaignMonthStr']);
                            }
                        ...
                        $("#eventTbody").append('<tr><td class="table-column-pe-0"><div class="form-check"><input eventListIndex="'+i+'" class="form-check-input" name="radios" type="radio" value="" id="usersDataCheck2">\
                        <label class="form-check-label" for="usersDataCheck2"></label></div></td><td class="ps-0">' + eventList[i]['Campaign'] + '</td>\
                        <td>' + eventList[i]['Reporting Business Unit'] + '</td>\
                        <td>' + eventList[i]['Category'] + '</td>\
                        <td>' + statusTd + '</td>\
                        <td>' + eventList[i]['Date'].split('T')[0] + '</td></tr>');
  ...
  $( "#eventTbody" ).on( "click", ".form-check-input", function() {
    ...
    //console.log("click:" + eventList[eventListIndex]['Campaign']);
    //console.log(eventList[eventListIndex]);
    //console.log("*****************************************");
	//console.log($(this).parent().parent().next().html());
	//console.log(eventListIndex);
    let eventListIndex =  $(this).attr("eventListIndex");
    var sends = numberWithCommas(eventList[eventListIndex]['Sends']);
    var opens = numberWithCommas(eventList[eventListIndex]['Opens']);
	//$("#sendsH5").html((sends<opens) ? opens: sends);//Deliveries
    $("#sendsH5").html(sends);//Deliveries
	$("#opensH5").html(opens);
    $("#ctrH5").html((eventList[eventListIndex]['CTR']* 100).toFixed(2)+'%');
    $("#mqlH5").html(numberWithCommas(eventList[eventListIndex]['MQL']));
    var tcvPipelineDollar = USDollar.format(Math.round(eventList[eventListIndex]['TCV Pipeline']));
	//$("#tcvPipelineH5").html(numberWithCommas(eventList[eventListIndex]['TCV Pipeline']));
    $("#tcvPipelineH5").html(tcvPipelineDollar);
    var tcvDollar = USDollar.format(Math.round(eventList[eventListIndex]['TCV']));
	$("#tcvH5").html(tcvDollar);
	$("#rsvpsH5").html(numberWithCommas(eventList[eventListIndex]['RSVPs']));
	$("#attendeesH5").html(numberWithCommas(eventList[eventListIndex]['ATTENDEES']));
	$("#invitedH5").html(numberWithCommas(eventList[eventListIndex]['INVITED']));
	$("#sharesH5").html(eventList[eventListIndex]['SHARES']);
	$("#reachH5").html(eventList[eventListIndex]['REACH']);
	$("#likesH5").html(eventList[eventListIndex]['LIKES']);
});
...
function doFilter(selectName){
    ...
    for (var i = 0, len = eventList.length; i < len; i++) {//console.log(collateralList[i]['Campaign']);
        ...
        if((eventList[i][columnName] != $(selectName).val() && $(selectName).val() != selectDefaultValue)){
            continue;
        }
        statusTd="";
        if(eventList[i]['Status'] == "Completed"){statusTd = '<div class="btn-status-table color-purple">Completed</div>';}
        if(eventList[i]['Status'] == "In Progress"){
            statusTd = '<div class="btn-status-table color-yellow">Running</div>';
            eventsInProgress++;
        }
        if(eventList[i]['Status'] == "Planned"){statusTd = '<div class="btn-status-table color-orange">Planned</div>';}
        ...
        invitedCount += parseInt(eventList[i]['INVITED']);
        rsvpsCount += parseInt(eventList[i]['RSVPs']);
        attendedCount += parseInt(eventList[i]['ATTENDEES']);
                        
                        
    }
    console.log("invitedCount:"+invitedCount);
    console.log("rsvpsCount:"+rsvpsCount);
    console.log("attendedCount:"+attendedCount);
    $(".form-check-input").eq(0).trigger("click");
    //================================================================================================
    $("#nav1").remove();
    $("#datatablePagination").parent().after('<div id="nav1"></div>');
...
$( "#content" ).on( "change", "#countrySelect", function() {//console.log("aaa"+$(this).val());
  ...
  //================================================================================================
    //$("#totalInitiatives").html(eventsInProgress+"/"+rowsTotal1);
    rsvpsAvgPct = Math.round((rsvpsCount/invitedCount)*100)+"%";
    attendedAvgPct = Math.round((attendedCount/rsvpsCount)*100)+"%";
    //console.log(attendedAvgPct);
