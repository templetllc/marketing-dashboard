...
if(!isset($_GET['s']) && !isset($_GET['e'])){    
    if(date("d", strtotime($date_now)) == 1){
        $startAux = mktime(0, 0, 0, date("m", $date_past)  , 16, date("Y", $date_past));
        $dateStart = date("Y-m-d", $startAux) . 'T00:00:00';
        $dateEnd = date("Y-m-d", $date_past) . 'T23:59:59';
    } elseif(date("d",  strtotime($date_now)) == 16) {
        $startAux = mktime(0, 0, 0, date("m"), 1, date("Y"));
        $dateStart = date("Y-m-d", $startAux) . 'T00:00:00';
        $dateEnd = date("Y-m-d", $date_past) . 'T23:59:59';
    } else {
        exit();
...
$fileDirServer = "/home/u348838306/domains/templet.io/public_html/clients/libertynet/marketing-dashboard/data/summary/";
$fileSummary = $fileDir.'summary_'.$_start.'_'.$_end.'.json';


$summaryString = json_encode($summary);
file_put_contents($fileSummary, $summaryString);

$fileSummary2 = $fileDirServer.'summary_'.$_start.'_'.$_end.'.json';
